 ...
 * contextObject(). Объект контекста не создается явно, а наследуется от
 * родителя. В случае, если родителя нет, объект создается при помощи функции
 * makeContext(). Реализация по умолчанию не создает объект и возвращает NULL.
 * Функцию следует реализовывать в объектах верхнего уровня - главном окне и
 * диалогах.
 *
...
public:
	 ...
	 *
	 * TODO источник ищется в контексте получателя (сюда передается имя),
	 * а должен - в контексте листа connect (сюда передается ссылка).
	 * @param obj имя объекта-получателя в контексте объекта
...
protected:
	 ...
	 * Функция создает контекст на основе заданного контекста (TODO).
	 * Контекст должны создавать объекты верхнего уровня (главное окно,
	 * диалоги). Реализация по умолчанию возвращает NULL.
	 * @param parent родительский контекст (TODO)
	 * @return контекст
	 * TODO всегда вызывать данную функцию (переименовать). Реализация по
	 * умолчанию должна возвращать переданный контекст, реализация для
	 * окон - создавать потомка.
	 */
...
public:
	 ...
	 * Удаляет регистрацию последнего заданного строителя для
	 * соответствующего имени класса. Если ранее для этого имени класса был
	 * задан еще один строитель, он становится видимым.
	 * TODO удаление по имени, а не описанию
	 * @param item описание строителя
	 */
